function [Delay,Delay1,rou,max_rou] = Mapping_Leader_follow_network_anis_factor_simplified(plot_LFT,Frame_matrix,tracks_filt,findRou,tau_threshold,correlation_threshold)
    ...
    for i = 1 : size(Delay1,1)
        for j = 1 : size(Delay1,2)
            if j ~= i
                ...
                plot(Delay(i,j),max_rou(i,j),'ro','MarkerFaceColor','r')
                ...
                line(ax.XLim,[correlation_threshold,correlation_threshold],'linestyle','--','color',hex2rgb('262626'))
                line([0 0],ax.YLim,'linestyle','--','color',hex2rgb('262626'))
            end
        end
    end
    set(gcf,'position',[107 582 1711 1022])
end
end